...
import { initDatabase, loadAllIntoMemory, flushToDatabase, closeDatabase } from './lib/db.js'
import {mongoDB, mongoDBV2} from './lib/mongoDB.js'
import store from './lib/store.js'
import { printLibcInfo } from './lib/libc.js'
...
const libcInfo = printLibcInfo(msg => console.log(chalk.bold.gray(msg)))
if (libcInfo.isMusl) {
    console.log(chalk.bold.yellowBright(`⚠︎ Entorno musl detectado, se requiere el prebuild ${libcInfo.target}`))
} else if (libcInfo.libc !== 'n/a') {
    console.log(chalk.bold.greenBright(`✓ Entorno glibc detectado, se usará el prebuild ${libcInfo.target}`))
}
...
throw new Error('El adaptador remoto (--db <url>) no está soportado por la capa SQLite. Quita esa opción o pide que se agregue soporte explícito.')
}

const sqlite = initDatabase('./database.sqlite')
global.db = {
data: null,
async read() { global.db.data = loadAllIntoMemory(sqlite) },
async write() { flushToDatabase(sqlite, global.db.data) },
}
...
async function shutdownDatabase(signal) {
try {
if (global.db.data) await global.db.write()
} catch (e) {
console.error('[db] fallo el flush final:', e)
}
...
const { blackJadiBot } = await import('./plugins/jadibot-serbot.js')
