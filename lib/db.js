...
const DEFAULTS = {
  users: {
    exp: 0, joincount: 1, diamond: 3, coin: 0, lastadventure: 0, lastclaim: 0,
    health: 100, crime: 0, lastcofre: 0, lastdiamantes: 0, lastpago: 0, lastcode: 0,
    lastcodereg: 0, lastduel: 0, lastmining: 0, premiumTime: 0, genre: '', birth: '',
    marry: '', description: '', name: '', age: -1, regTime: -1, afk: -1, afkReason: '',
    role: 'Nuv', bank: 0, warn: 0, wins: 0, losses: 0, draws: 0, spam: 0,
  },
  chats: {
    sAutoresponder: '',
  },
...
function upsertRow(db, tableName, key, obj) {
  ...
  const defaults = DEFAULTS[tableName] || {}
  for (const col of columns) {
    if (!(col in values)) {
      values[col] = bool.includes(col) ? 0 : (col in defaults ? defaults[col] : null)
    }
  }
