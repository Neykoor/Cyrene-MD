...
END:VCARD
        ...
        var ab = await baper.resize({ w: ukur1, h: ukur2 }).getBuffer(JimpMime.jpeg)
        resolve(ab)
       })
      }
    },
    ...
    async value(jid, button, rows, quoted, options = {}) {
     ...
     if (buffer) try { (type = await conn.getFile(buffer), buffer = type.data) } catch { buffer = buffer }
          ...
          forwardedNewsletterMessageInfo: {
            ...
            newsletterName: 'Roxy -MD ¡New Channe! 🍂', 
            serverMessageId: '' }, 
                ...options
                }
                })
...
reply: {
      ...
      */
      ...
      async value(image, width, height) {
       ...
       let kiyomasa = await oyy.resize({ w: width, h: height }).getBuffer(JimpMime.jpeg)
       return kiyomasa
      }
    },
    /** Profile Image
    ...
    async value(buffer) {
        ...
        const resz = jimp_1.width > jimp_1.height ? jimp_1.resize({ w: 550 }) : jimp_1.resize({ h: 650 })
        const jimp_2 = await Jimp.read(await resz.getBuffer(JimpMime.jpeg));
        return {
          img: await resz.getBuffer(JimpMime.jpeg)
        }}
        },
...
return conn.sendMessage(jid, { poll: { name, values, selectableCount }})
    ...
    conn.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: file, templateButtons: but, ...options })
        ...
        parseMention: {
             ...
             */
            value(text = '') {
                // Acepta @123456789 o @123456789@lid (normaliza ambos)
                return [...text.matchAll(/@([0-9]{5,16})(?:@lid)?/g)].map(v => v[1] + '@s.whatsapp.net')
            },
            enumerable: true
        },
        getName: {
...
export function serialize() {
    ...
    return (this?.fromMe || areJidsSameUser(this.conn?.user.id, this.sender)) && this.id.startsWith('3EB0') && (this.id.length === 20 || this.id.length === 22 || this.id.length === 12) || false
                ...
                return Object.defineProperties(JSON.parse(JSON.stringify(typeof q === 'string' ? { text: q } : q)), {
                    ...
                    download: {
                        async value(saveToFile = false) {
                            if (!this.mediaType) return Buffer.alloc(0)
                            const buffer = await downloadMediaMessage(this.vM, 'buffer', {})
                            if (!saveToFile) return buffer
                            const { filename } = await self.conn?.getFile(buffer, true)
                            return filename
                        },
                        enumerable: true,
                        configurable: true,
        ...
        download: {
            async value(saveToFile = false) {
                if (!this.mediaType) return Buffer.alloc(0)
                const buffer = await downloadMediaMessage(this, 'buffer', {})
                if (!saveToFile) return buffer
                const { filename } = await this.conn?.getFile(buffer, true)
                return filename
            },
            enumerable: true,
            configurable: true
