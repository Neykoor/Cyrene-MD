...
let db: any;
let insertCharacterStmt: any;
let findByTagStmt: any;
let randomCharacterStmt: any;
let getEnabledStmt: any;
let setEnabledStmt: any;

function buildStatements(): void {
  if (db) {
    try {
      db.close();
    } catch {
  ...
  insertCharacterStmt = db.prepare<unknown, [string, string, string, string, string, number, number]>(
    "INSERT OR IGNORE INTO gacha_characters (name, series, gender, booru_tag, image_url, value, created_at) VALUES (?, ?, ?, ?, ?, ?, ?)"
  );

  findByTagStmt = db.prepare<CharacterRow, [string]>("SELECT * FROM gacha_characters WHERE booru_tag = ?");

  randomCharacterStmt = db.prepare<CharacterRow, []>("SELECT * FROM gacha_characters ORDER BY RANDOM() LIMIT 1");

  getEnabledStmt = db.prepare<EnabledRow, [string]>("SELECT enabled FROM gacha_group_config WHERE chat_id = ?");

  setEnabledStmt = db.prepare<unknown, [string, number, string, number]>(
...
function isFinalizedError(err: any): boolean {
  const message = String(err?.message || err || "").toLowerCase();
  return message.includes("finalized");
}
...
function withRetry<T>(fn: () => T): T {
  try {
    ...
    if (!isFinalizedError(err)) throw err;
    buildStatements();
    return fn();
  }
}
...
export function characterExists(booruTag: string): boolean {
  return withRetry(() => !!findByTagStmt.get(booruTag));
}
...
export function getRandomCharacter(): CharacterRow | undefined {
  return withRetry(() => randomCharacterStmt.get());
}
