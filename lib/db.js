...
// Columnas "core" por tabla (todo lo que NO esté aquí se guarda/lee de la
// columna `extra` como JSON, así un plugin puede seguir agregando toggles
// sin que se necesite un ALTER TABLE cada vez).
const TABLES = {
  users: {
    pk: 'jid',
...
function quoteCol(col) {
  // `delete` es palabra reservada en varios dialectos SQL; se guarda entre comillas.
